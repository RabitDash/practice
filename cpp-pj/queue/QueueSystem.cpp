...
void QueueSystem::customerDeparture() {
	if (current_event->occure_time < total_service_time) {
		total_customer_stay_time += cuurent_event->occure_time - windows[current_event->event_type].getCustomerArriveTime();
	...
	if(customer_list.length()) {
		...
		Event temp_event(
			current_event->occur_time + customer->duration, current_event->event_type);
		event_list.orderEnqueue(temp_event);
		delete customer();
	} else {
